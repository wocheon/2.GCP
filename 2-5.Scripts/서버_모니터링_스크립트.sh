...
bg_grey "#Server Spec"
echo "$(yellow vCPU) : $(grep -c processor /proc/cpuinfo)"
echo "$(yellow Memory) : $(free -h | grep Mem | gawk '{print $2}')"
echo ""
...
use_per=$(echo "scale=2; $use/ $total * 100" | bc)
use_per2=$(echo "scale=2; ($total -$avail) / $total * 100" | bc)
#echo "Memory Usage(%) : $use_per %"
bg_blue "Memory Usage(%) : $use_per2 %"
echo ""
